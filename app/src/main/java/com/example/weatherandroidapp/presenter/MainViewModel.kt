...
class MainViewModel @Inject constructor(val repository: MainRepository) : ViewModel() {
    ...
    fun UVDescription(UV :Int):String{
        when(UV){
            in 0..2-> return "УФ Низкий"
            in 3..5-> return "УФ Средний. Рекомендуется использовать крем c spf"
            in 6..7-> return "УФ Высокий. Не забудьте про крем c spf"
            in 8..10-> return "УФ Очень высокий. Используйте крем c spf"
            else-> return "УФ Экстремальный. Апокалипсис случился. "
        }
    }
